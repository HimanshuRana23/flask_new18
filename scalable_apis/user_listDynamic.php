...
Created for : This api is used to get Users List Dynamically based on user resources from user table
URL : https://flask.dfos.co/scalable_apis/user_listDynamic.php?server=hero&table=users&columns=user_id as id, firstname, lastname&match_column=user_resources&deleted_flag=1
 Method      : POST	

Description : This API dynamically fetches user list based on the provided option_id, match_column, and other filters passed via POST. It's designed for flexible dropdown population using table, column, condition, and grouping parameters.
	...
	try { 
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {  
			...
			if ($resource_id!='') {   
                ...
                if($match_column){
                    $query .= " WHERE FIND_IN_SET('$resource_id', $match_column)";
                }
                if($deleted_flag){
