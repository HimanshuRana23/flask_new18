...
Created for : This api is used to fetch data from single table dynamically
URL : For eg https://flask.dfos.co/scalable_apis/dynamicapi.php?server=heromotocop&table=form_via_form_main_audit_answers&columns=fvf_main_audit_id,answer&conditions={"fvf_main_field_id":"4641411"}&deleted_at=1&group_by=answer
Type : Get
Description :This dynamic API is used to fetch data from a single table using flexible GET parameters.
It supports:

Custom table and columns selection

Optional WHERE conditions as JSON (conditions)

Soft-delete filtering via deleted_at=1

Grouping using group_by

First column is auto-aliased as id for consistent formatting

Returns raw result array in JSON
*/
...
try {
    ...
    $deleted_flag = isset($_GET['deleted_at']) ? $_GET['deleted_at'] : 0; // 1 = Include deleted_at condition
