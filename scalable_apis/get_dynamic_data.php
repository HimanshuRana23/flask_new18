...
/*
Created By : @maulik - 21032025
Created for : Api to fetch employee_id of all the users
*/
...
try {
    ...
    if (!empty($conditions)) {
        ...
        foreach ($conditions as $column => $value) {
            if (preg_match('/^[a-zA-Z0-9_ \'\>\<\=!\.]+$/', $column)) { 
                // Allowing SQL operators like !=, =, >, <, etc.
                if ($value === true) {
                    // If value is just a flag to indicate raw condition
                    $conditionStrings[] = "$column";
                } else {
