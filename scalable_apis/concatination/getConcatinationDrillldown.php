...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        while ($row = mysqli_fetch_assoc($sqldata)) {
            ...
            if (!isset($answerData[$fieldId])) {
                $answerData[$fieldId] = [
                    'answers' => [],
                    'ansIds' => []
                ];
            }
            $answerData[$fieldId]['answers'][] = $answer;
            $answerData[$fieldId]['ansIds'][] = $ansId;
        ...
        if (!empty($answerData)) {
            // Get the first field's data to determine the number of entries
            $firstFieldData = reset($answerData);
            $maxLength = count($firstFieldData['answers']);

            for ($i = 0; $i < $maxLength; $i++) {
                $combinedAnswers = [];
                $currentAnsId = null;
                
                foreach ($answerData as $fieldId => $data) {
                    $combinedAnswers[] = $data['answers'][$i] ?? '';
                    // Use the ansId from the first field (or you might need to combine them differently)
                    if ($currentAnsId === null) {
                        $currentAnsId = $data['ansIds'][$i] ?? null;
                    }
                }

                $result[] = [
                    "fvf_main_field_option_id" => $currentAnsId,
                    "fvf_main_field_type" => "DrillDown",
                    "fvf_main_field_option_name" => implode(' - ', $combinedAnswers),
